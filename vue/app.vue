...
export default {
  ...
  methods: {
    ...
    async createSection () {
      // gets all sections and assigns their name option to an allNames array
      this.getAllSections()
      // checking whether there is duplicate names + check whether input field is not empty
      const foundName = this.allNames.find(e => e === this.form.name)
      if (!this.form.name) {
        this.form.isValid = false
      } else if (typeof (foundName) !== 'undefined') {
        ...
        this.sectionId = ''
        await this.getLastSection(lastIndex)
        this.handleModal()
      }
    },
    async getLastSection (lastIndex) {
      await this.$uci.load('task_one')
      const newSectionId = this.$uci.sections('task_one', 'test')[lastIndex][
        '.name'
      ]
